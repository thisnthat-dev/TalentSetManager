...
local function updateList()
 ...
 if #sets > 0 then
  for k,v in pairs(sets) do
   ...
   h = h + 20
   f:SetPoint("TOPLEFT", mf, "TOPLEFT", 5, ((k - 1) * -20) - 6)   
   f:SetText(v.name)
  ...
  f.data = nil
  f:SetText(format(L["no_talent_sets"], "|cff00ff00"..(tt == "talents_pvp" and COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATEPVP or COMPACT_UNIT_FRAME_PROFILE_AUTOACTIVATEPVE).."|r"))
  f:SetPoint("TOPLEFT", mf, "TOPLEFT", 5, -5)
  f.equipmentTexture:Hide()
  f.Check:Hide()
  f.Stripe:Hide()
  f.Icon:SetTexture(nil)
...
local function showFrame(p)
 ...
 if not mf then
  mf = CreateFrame("Frame", "TalentSetManagerLDBFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate")
  mf:EnableMouse(true)
...
--  end
 ...
 mf:ClearAllPoints()
 mf:SetPoint("TOPLEFT", p, "BOTTOMLEFT", 0, 0)
--  mf:SetPoint("TOPLEFT", p, "BOTTOMLEFT", -10, 0)
--  mf:SetPoint("BOTTOMLEFT", p, "TOPLEFT", -10, 0)
